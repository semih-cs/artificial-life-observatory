...
const USAGE = 'usage: world --dir <path> [--new --seed <uint32>] [--save-every <ticks>] [--keep <n>] [--until-tick <tick>] [--status-every <ticks>] [--json]';
...
function main(): void {
  ...
  const started = runner.status();
  const recovery = runner.recovery;
  out('started', {
    ...started,
    skipped: recovery?.report.skipped ?? [],
    quarantined: recovery?.quarantine?.moved ?? [],
  }, `${runner.origin === 'fresh' ? 'created' : `recovered from snapshot ${started.recoveredFromTick}`} in ${args.dir} | ${line(started)}`
  ...
  const onSignal = (sig: NodeJS.Signals) => {
    signals++;
    if (signals > 1) {
      process.stderr.write(`[world] second ${sig}: exiting now; the newest saved snapshot (${runner.snapshotTick}) is intact\n`);
      process.exit(130);
    }
    out('stopping', { signal: sig, tick: runner.tick }, `${sig}: stopping after tick ${runner.tick}, saving…`);
    runner.stop();
  };
  ...
  runner.run({ untilTick: args.untilTick, statusEvery: args.statusEvery ?? args.saveEvery, onStatus })
    .then((result) => {
      out('stopped', { ...result, ...runner.status() }, `stopped (${result.reason}) at tick ${result.tick}; saved snapshot ${result.snapshotTick}`);
      process.exit(0);
    })
    .catch((err: unknown) => {
      process.stderr.write(`error: ${err instanceof Error ? err.message : String(err)}\nthe newest saved snapshot (${runner.snapshotTick}) is intact\n`);
      process.exit(1);
    });
}
